<html lang="ru">
<head>
    <title>Конкретный пользователь</title>
</head>
<body>
<?php
session_start();
...
if (isset($_POST["Name"])) {
    ...
    if (isset($_GET['red_id'])) {
        $sql = mysqli_query($conn, "SELECT `id`, `name`, `surname`,`login`,`password`,`lang`,`role` FROM `users` WHERE `id`={$_GET['red_id']}");
        $sql = mysqli_query($conn, "UPDATE `users` SET `name` = '{$_POST['Name']}',`surname` = '{$_POST['Surname']}',`login` = '{$_POST['Login']}',
                                                            `password` = '{$_POST['Password']}',`lang` = '{$_POST['Language']}',`role` = '{$_POST['Role']}' WHERE `id`={$_GET['red_id']}");
    } else {

        $sql = mysqli_query($conn, "INSERT INTO `users` (`name`, `surname`,`login`,`password`,`lang`,`role`) VALUES ('{$_POST['Name']}', '{$_POST['Surname']}','{$_POST['Login']}',
...
<table border = '1' >
    <tr >
        <td > id</td ><td > Name</td ><td > Surname</td ><td > Login</td ><td > Password</td ><td > Language</td ><td > Role</td ><td > Редактирование</td >
    </tr >

    <?php
    $sql = mysqli_query($conn, "SELECT `id`, `name`, `surname`,`login`,`password`,`lang`,`role` FROM `users` WHERE `id` = {$_SESSION['user']['id']}");
    while ($result = mysqli_fetch_array($sql)) {
        echo '<tr>' .
            "<td>{$result['id']}</td>" . "<td>{$result['name']}</td>" . "<td>{$result['surname']}</td>" . "<td>{$result['login']}</td>" . "<td>{$result['password']}</td>" .
